...
void Sys_PowerModes_Wakeup_WithReset(sleep_mode_cfg *p_sleep_mode_cfg)
{
    ...
    /* If XTAL is not disabled after wakeup */
    if(!((p_sleep_mode_cfg->boot_cfg & ACS_BOOT_CFG_BOOT_SELECT_Mask) == BOOT_FLASH_XTAL_DISABLE))
    {
    ...
    Sys_Clocks_DividerConfig(p_sleep_mode_cfg->clock_cfg.uartclk_freq,
                             p_sleep_mode_cfg->clock_cfg.sensorclk_freq,
                             p_sleep_mode_cfg->clock_cfg.userclk_freq);
...
static void _Sys_PowerModes_Standby_WakeupInit(standby_mode_cfg *p_standby_mode_cfg)
{
    ...
    /* If XTAL is not disabled after wakeup */
    if(!((p_standby_mode_cfg->boot_cfg & ACS_BOOT_CFG_BOOT_SELECT_Mask) == BOOT_FLASH_XTAL_DISABLE))
    {
        while ((RF0_ANALOG_INFO->ANALOG_INFO_BYTE & ANALOG_INFO_CLK_DIG_READY_BYTE) != ANALOG_INFO_CLK_DIG_READY_BYTE)
        {
            SYS_WATCHDOG_REFRESH();
    ...
    /* Call the Divider Config after the SystemCoreClock global variable is updated */
    Sys_Clocks_DividerConfig(p_standby_mode_cfg->clock_cfg.uartclk_freq,
                             p_standby_mode_cfg->clock_cfg.sensorclk_freq,
                             p_standby_mode_cfg->clock_cfg.userclk_freq);
...
static void _Sys_PowerModes_Wakeup_ClockInit(clock_cfg *p_clock_cfg, uint32_t boot_cfg)
{
        ...
        ((boot_cfg & ACS_BOOT_CFG_BOOT_SELECT_Mask) == BOOT_FLASH_XTAL_CUSTOM_TRIM))
    {
        ...
        RF0_REG33->CK_DIV_1_6_CK_DIV_1_6_BYTE = systemclk_div;
    }
